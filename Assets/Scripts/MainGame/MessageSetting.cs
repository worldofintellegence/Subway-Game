...
public class MessageSetting : MonoBehaviour
{
    ...
    {
        int lang = Modules.indexLanguage;
        callPlane.GetComponent<PlaneController>().RemoveAllPlane();
        if (Modules.reducedEffect == 2)
        {
            Modules.reducedEffect = 1;
            valueReducedEffect.text = AllLanguages.menuMedium[lang];
            callPlane.GetComponent<PlaneController>().CallPlaneStart();
        }
        ...
        else
        {
            ...
            callPlane.GetComponent<PlaneController>().RemoveCallPlane();
        }
        Modules.SaveSettingValue();
        Modules.PlayAudioClipFree(Modules.audioButton);
    }
    ...
    {
        Application.OpenURL("https://www.facebook.com/EGaming-205176820063908/");
    }
