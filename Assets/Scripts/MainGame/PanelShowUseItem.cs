...
public class PanelShowUseItem : MonoBehaviour {
    ...
    void FixedUpdate()
    {
        ...
        if (removingPanel) return;
        if (runTimeCool >= totalTime)
        {
            if (codeItemNow == TypeItems.hoverboard)//xu ly van truot phai hoan thanh xong dong tac nhay
            {
                if (!Modules.mainCharacter.GetComponent<HeroController>().IsFinishJump())
                    return;
            }
            if (handleContent)
            {
                ...
                if (codeItemNow == TypeItems.hoverboard)//van truot
                    Modules.mainCharacter.GetComponent<HeroController>().RemoveSkisItem(true, finishNormal);
                ...
                else if (codeItemNow == TypeItems.doubleCoin)//double coin
                    Modules.mainCharacter.GetComponent<HeroController>().RemoveDoubleCoinItem();
            }
        ...
        {
            if (codeItemNow != TypeItems.jetpack && codeItemNow != TypeItems.hoverbike && codeItemNow != TypeItems.jetball)//neu la cac item khac rocket va cable thi set them dieu kien
            {
                if (!Modules.useRocket && !Modules.useJumper && !Modules.useCable && !Modules.useJetBall && Modules.statusGame == StatusGame.play)
                    runTimeCool++;
            }
            else runTimeCool++;
    ...
    public void ResetTime(Vector2 newTime)
    {
        ...
        finishNormal = true;
        handleContent = true;
        CancelInvoke("DestroyPanel");
    }

    private bool handleContent = true;
    private bool finishNormal = true;
    ...
    private bool removingPanel = false;
    ...
    {
        if (runTimeCool >= totalTime && !removingPanel) return true;
        else return false;
    }
}
