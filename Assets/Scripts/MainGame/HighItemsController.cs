...
public class HighItemsController : MonoBehaviour {
    ...
    public List<GameObject> listParBoom;
                        ...
                        if (runCoinLane == Mathf.RoundToInt(rangeChange.x / 2f))
                        {
                            ...
                            if (ran < percentItems || Modules.useJumper)//neu dung ty le hoac dang an jumper thi xuat hien
                            {
                                ...
                                for (int j = 0; j < listItems.Count; j++)
                                    ...
                                    {
                                        if (listItems[j].GetComponent<ItemInformation>().typeItem == TypeItems.hoverbike && !Modules.allowHoverbike) continue;
                                        if (!Modules.useJumper)
                                        {
                                            if (listItems[j].GetComponent<ItemInformation>().typeItem != TypeItems.jetpack
                                                ...
                                                && listItems[j].GetComponent<ItemInformation>().typeItem != TypeItems.jetball)
                                                indexItems.Add(j);
                                        }
                                        else
                                        {
                                            //xu ly neu la cac item bay thi giam ty le di 50% theo yeu cau moi (neu sau nay muon can bang cac item thi bo)
                                            if (listItems[j].GetComponent<ItemInformation>().typeItem == TypeItems.jetpack
                                                || listItems[j].GetComponent<ItemInformation>().typeItem == TypeItems.hoverbike
                                                || listItems[j].GetComponent<ItemInformation>().typeItem == TypeItems.jetball)
                                            {
                                                int ranFly = Random.Range(0, 100);
                                                if (ranFly < 50) indexItems.Add(j);
                                            }
    ...
    public void PlayEffectEatItems(Vector3 pointShow, bool playAudio = true)
    {
        foreach (GameObject go in listParEatItems)
        {
            ...
            {
                go.transform.position = pointShow;
                go.GetComponent<PlayParticle>().Play(false, playAudio);
                break;
            }
        }
    }

    public void PlayEffectBoom(Vector3 pointShow)
    {
        foreach (GameObject go in listParBoom)
        {
            if (!go.GetComponent<PlayParticle>().IsPlay())
            {
                go.transform.position = pointShow;
