...
public class TerrainInformation : MonoBehaviour {
    ...
    public bool isWaterTerrain = false;
    //xu ly hieu ung gia fog mat nuoc
    public bool fakeFogWater = false;
    public float highFogWater = -2.5f;
    //xu ly cac hieu ung khac
    public GameObject objectRotate;//xu ly map co crossroads
    ...
    public void Restart(bool handleSafe = true)
    {
        ...
        if (objectRotate != null)
            objectRotate.transform.eulerAngles = Vector3.zero;
        ...
        if (objectWater != null)
        {
            if (Modules.reducedEffect > 0) objectWater.SetActive(false);
            else objectWater.SetActive(true);
        }
        if (handleSafe)
        {
            runReset = 0;
    ...
    private int handelPerTime = 2;//moi lan update xu ly 10 doi tuong
    void Update()
    {
