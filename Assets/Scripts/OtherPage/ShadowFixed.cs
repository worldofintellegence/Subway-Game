...
public class ShadowFixed : MonoBehaviour {
	...
	void Update () {
        ...
        //oldTime = Time.time;
        if (Modules.statusGame == StatusGame.bonusEffect && !Modules.useTemple) { shadowShow.SetActive(false); return; }
        if (Physics.Raycast(pointRaycast.transform.position, Vector3.down, out hit, 50f, colliderAllow))
        {
            ...
            shadowShow.transform.position = new Vector3(hit.point.x, hit.point.y + 0.2f, hit.point.z);
            shadowShow.transform.rotation = Quaternion.Euler(new Vector3(hit.collider.transform.localEulerAngles.x, 0, 0));
        }
