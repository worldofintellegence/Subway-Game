...
public class ButtonUpgradesItem : MonoBehaviour {
    ...
    public void ButtonClick()
    {
        ...
        Modules.PlayAudioClipFree(Modules.audioUpgrade);
        ...
        else if (codeItem == 6)//neu la jetball
        {
            ...
            if (Modules.totalCoin >= cost)//neu du tien
            {
                if (Modules.levelUpgradeJetBall < Modules.maxLevelItem)
                {
                    Modules.levelUpgradeJetBall++;
                    Modules.totalCoin -= cost;
                    Modules.SaveCoin();
                    textCoin.text = Modules.totalCoin.ToString();
                    Modules.SaveLevelJetBall();
                    transform.parent.parent.gameObject.GetComponent<UpgradesController>().LoadDataNow();
                }
                else
                    ShowError(AllLanguages.shopMaxLevel[Modules.indexLanguage]);
        ...
        else if (codeItem == 7)//neu la double coin
        {
            ...
            if (Modules.totalCoin >= cost)//neu du tien
            {
                if (Modules.levelUpgradeDouble < Modules.maxLevelItem)
                {
                    Modules.levelUpgradeDouble++;
                    Modules.totalCoin -= cost;
                    Modules.SaveCoin();
                    textCoin.text = Modules.totalCoin.ToString();
                    Modules.SaveLevelDouble();
                    transform.parent.parent.gameObject.GetComponent<UpgradesController>().LoadDataNow();
                }
                else
                    ShowError(AllLanguages.shopMaxLevel[Modules.indexLanguage]);
