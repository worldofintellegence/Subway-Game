...
public class UpgradesController : MonoBehaviour {
    ...
    public void LoadDataNow()
    {
        ...
        else if (codeItem == 6)//neu la jetball
        {
            float money = 250 * Mathf.Pow(2, (Modules.levelUpgradeJetBall + 1));
            if (money > maxCost) money = maxCost;
            textCost.text = money.ToString();
            progressBox.fillAmount = (float)Modules.levelUpgradeJetBall / (float)Modules.maxLevelItem;
        }
        else if (codeItem == 7)//neu la double coin
        {
            float money = 500 * Mathf.Pow(2, (Modules.levelUpgradeDouble + 1));
            if (money > maxCost) money = maxCost;
            textCost.text = money.ToString();
            progressBox.fillAmount = (float)Modules.levelUpgradeDouble / (float)Modules.maxLevelItem;
        }
    }
}
